...
def compute_representative_patterns(tricluster, categorical_feats, continuos_feats):
    ...
    def aux_mode(z):
        if len(z) == 0:
            ...
            return ss.mode(z)[0][0]
    
    result = []
    for tp in tricluster.getTimes():
        bic_vals = list()
        bicluster = [tricluster.getFeatValues(tp, f) for f in tricluster.getSamples()]
...
def compute_distance_matrix(patients_tri, triclusters, categorical_feats=[], continuos_feats=[], corr=False):
    ...
    p_pats_reps=list()
    for bic_p in patients_tri:
        p_pats = [bic_p.getPatientsVals(t=tp) for tp in bic_p.getTimes()]  #Compute Patient Patterns
    ...
    for tric in triclusters:
        # print(i)
        # print(len(tric.getPatients()))
        t_pats = compute_representative_patterns(tric, categorical_feats, continuos_feats)
    ...
    # print(triclusters[0].cluster["T-0"])
        ...
        for t in t_pats_reps:
            ...
            for i in range(len(t)):
                ...
                if not corr:
                    ...
                    if len(a)==2:
                        dist_set[(pat, tric_names[nTr], i)] = dot_product(list(a),list(b))
                    else:
                        dist_set[(pat, tric_names[nTr], i)] = pearson_correlation(list(a),list(b))
            nTr +=1
        pat += 1
...
def write_matrix_static(static_feats_matrix, final_matrix, file_name, classes):
    cols = ["Gender", "BMI", "MND familiar history", 'Age at onset'] + ["TRI"+str(n) for n in range(len(final_matrix[0]))]
    final_final_matrix = np.concatenate((static_feats_matrix, final_matrix), axis=1)
...
def pearson_correlation(numbers_x, numbers_y):
	...
	dem = (math.sqrt(sum(subtracted_mean_square_x)) * math.sqrt(sum(subtracted_mean_square_y)))
